...
public class Lerp1 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i =0; i < numSphere; i++){
            ...
            startPosition[i] = new Vector3(Mathf.Sqrt(2) * MathF.Pow(Mathf.Sin(i), 3), Mathf.Pow(-Mathf.Cos(i), 3) - Mathf.Pow(Mathf.Cos(i), 2) + 2 * Mathf.Cos(i));

            float r = 10f;
            //random end positions
            endPosition[i] = new Vector3(r * UnityEngine.Random.Range(-1f, 1f), r * UnityEngine.Random.Range(-1f, 1f), r * UnityEngine.Random.Range(-1f, 1f));
        }
    ...
    void Update()
    {
        ...
        for (int i =0; i < numSphere; i++){
            ...
            spheres[i].transform.position = Vector3.Slerp(startPosition[i], endPosition[i], lerpFraction);
